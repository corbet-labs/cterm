...
| File transfer over the TTY | Partial | Bounded OSC 5113 codec/queue, exact consent tokens, cancel/quiet policy, and consent-gated private handle-relative regular-file staging with strict zlib, metadata, limits, and per-file atomic commit | Native consent integration, nonblocking execution, receive sessions, directory/link trees, rsync/XXH3, bypass policy, and Kitty differential tests |
