...
| File transfer over the TTY | Partial | Kitty-compatible unpadded codec, lossless bounded daemon actor, exact replayable consent tokens, owned deny-default Cocoa/GTK/Win32 prompts with expiry and lifecycle cancellation, relaunch-safe cleanup, private handle-relative regular-file send staging with strict zlib, cumulative limits, metadata, missing-parent creation, and per-file atomic commit, plus recursive consent-listed receive trees with retained directory/file handles, symlink and hardlink correlation, bounded plain/zlib streaming, and PTY backpressure | Native prompt interaction automation, remote-to-local directory/link creation, rsync/XXH3, bypass policy, and Kitty differential tests |
